...
class FireAlarmListener:
    def __init__(self, model_path='model/model.pt', rf_path='model/rf_model.pkl'):
        ...
        self.uncertainty_threshold = 0.80  # If confidence < 65%, save it for me to manually inspect
        os.makedirs("active_learning_pool", exist_ok=True)
    ...
    def audio_callback(self, indata, frames, time_info, status):
        ...
        if is_danger and conf > 0.80:
            ...
            print("⚠️ POTENTIAL DANGER - Low Confidence. Saving for review.")
            self.hits += 1 
            self.save_uncertain_sample(seg, f"DANGER_{predicted_class}")
        elif conf < self.uncertainty_threshold:
            print("🤔 Uncertain sound. Saving for Active Learning.")
            self.hits = 0
            self.save_uncertain_sample(seg, "uncertain")
        else:
            self.hits = 0
    ...
    def save_uncertain_sample(self, audio, prefix):
        timestamp = int(time.time())
        filename = f"active_learning_pool/{prefix}_{timestamp}.wav"
