...
PUBLIC_BASE_URL = os.getenv("PUBLIC_BASE_URL", "http://10.194.183.127:8000")
...
def notify_hub_users_push(hub_id: int, message: str, notification_data=None):
    with get_db() as conn:
        cursor = conn.cursor()
        cursor.execute(
            """
...
def upload_audio():
    ...
    return jsonify({
        ...
        "audio_url": make_audio_url(unique_name)
    }), 201
...
def create_device_event(device_id):
    ...
    with get_db() as conn:
        ...
        return jsonify({
            ...
            "detected_at": to_new_york_time(row["detected_at"])
        }), 201
...
def create_alert():
    ...
    with get_db() as conn:
        ...
        cursor.execute(
            """
            SELECT de.device_event_id, de.device_id, de.event_type, de.detected_at
            FROM device_event de
            WHERE de.device_event_id = ?
            """,
            (device_event_id,)
        )
        ...
        cursor.execute(
            "INSERT INTO alert (hub_id, device_event_id, status, audio_url) VALUES (?, ?, ?, ?)",
            (hub_id, device_event_id, status, audio_url)
        )
        alert_id = cursor.lastrowid

    detected_at_ny = to_new_york_time(detected_at)
    ...
    notification_data = {
        ...
        "alarm_datetime": detected_at_ny,
        "detected_at": detected_at_ny,
        "audio_url": audio_url,
    }

    sms_notify_results = notify_hub_members(hub_id, msg)
    push_notify_results = notify_hub_users_push(hub_id, msg, notification_data)
...
def get_alert(alert_id):
    with get_db() as conn:
        ...
        detected_at_ny = to_new_york_time(row["detected_at"])
...
def get_alert_history(hub_id):
    ...
    with get_db() as conn:
        ...
        alerts = [{
            ...
            "event_type": row["event_type"],
            "detected_at": to_new_york_time(row["detected_at"]),
            "audio_url": row["audio_url"] if row["audio_url"] else None
        } for row in cursor.fetchall()]
...
def test_alert(hub_id):
    with get_db() as conn:
        ...
        cursor.execute(
            "SELECT detected_at FROM device_event WHERE device_event_id = ?",
            (device_event_id,)
        )
        detected_row = cursor.fetchone()
        detected_at = detected_row["detected_at"] if detected_row else datetime.utcnow().isoformat()

        cursor.execute(
            "INSERT INTO alert (hub_id, device_event_id, status, audio_url) VALUES (?, ?, 'pending', ?)",
            (hub_id, device_event_id, None)
        )
        alert_id = cursor.lastrowid
